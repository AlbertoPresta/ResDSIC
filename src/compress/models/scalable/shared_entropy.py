...
class ResWACNNSharedEntropy(WACNN):
    ...
    def forward(self, x, quality = None):
        ...
        scales_prog = self.h_scale(z_hat_prog)
        means_prog = self.h_mean(z_hat_prog)
        ...
        for j,p in enumerate(list_quality): 
            ...
            for slice_index, y_slice in enumerate(y_slices):
                ...
                support_prog_slices = (y_hat_slices if self.max_support_slices < 0 else y_hat_prog[:self.max_support_slices])
                ...
                # encode the progressive latent representation 
                mean_support = torch.cat([means_prog] + support_prog_slices, dim=1)
                mu_prog = self.cc_mean_transforms[slice_index](mean_support)
                mu_prog = mu_prog[:, :, :y_shape[0], :y_shape[1]]

                scale_support = torch.cat([scales_prog] + support_prog_slices, dim=1)
                scale_prog = self.cc_scale_transforms[slice_index](scale_support)
                scale_prog = scale_prog[:, :, :y_shape[0], :y_shape[1]]

                y_prog_slice = y_prog_slice - mu_prog 
                y_prog_slice = y_prog_slice*block_mask

                _, y_slice_likelihood_prog = self.gaussian_conditional_prog(y_slice, scale_prog, mask = block_mask)
            ...
            y_hat_prog = torch.cat(y_hat_prog,dim = 1)
            y_hat_progressive.append(y_hat_prog.unsqueeze(0))
    ...
    def compress(self, x, quality = 0.0):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            #support_index = min(max(0,slice_index - self.max_support_slices),self.max_support_slices)
            support_slices = (y_hat_slices if self.max_support_slices < 0 else y_hat_slices[:self.max_support_slices])
            support_slices_prog = (y_hat_slices if self.max_support_slices < 0 else y_hat_prog[:self.max_support_slices])
            ...
            mean_support = torch.cat([means_prog] + support_slices_prog, dim=1)
            mu_prog = self.cc_mean_transforms[slice_index](mean_support)
            mu_prog = mu_prog[:, :, :y_shape[0], :y_shape[1]]

            scale_support = torch.cat([scales_prog] + support_slices, dim=1)
            scale_prog = self.cc_scale_transforms[slice_index](scale_support)
            scale_prog = scale_prog[:, :, :y_shape[0], :y_shape[1]]
            ...
            y_hat_prog_slice = y_hat_prog_slice*block_mask

            y_q_slice_prog = self.gaussian_conditional_prog.quantize(y_hat_prog_slice*block_mask, "symbols")

            y_hat_prog_slice = y_q_slice_prog + mu_prog
