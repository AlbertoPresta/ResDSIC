...
class TCM(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2],
        ...
        self.N = N
        self.M = M
        self.num_slices = self.M//self.dim_chunk
        ...
        self.m_up1 = [ConvTransBlock(self.N, self.N, self.head_dim[3], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[3])] + \
                      [ResidualBlockUpsample(2*N, 2*N, 2)]
        self.m_up2 = [ConvTransBlock(self.N, self.N, self.head_dim[4], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[4])] + \
                      [ResidualBlockUpsample(2*N, 2*N, 2)]
        self.m_up3 = [ConvTransBlock(self.N, self.N, self.head_dim[5], self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
        ...
        self.ha_down1 = [ConvTransBlock(self.N, self.N, 32, 4, 0, 'W' if not i%2 else 'SW') 
                      for i in range(config[0])] + \
