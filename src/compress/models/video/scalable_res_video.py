...
class ResScalableScaleSpaceFlow(ScaleSpaceFlow):
    def __init__(
        ...
        assert motion_input in ("base","hybrid","total")
        ...
        if self.motion_input == "total": 

            self.motion_encoder = nn.ModuleList(Encoder(2*3) for _ in range(2))   
            self.motion_decoder = nn.ModuleList( Decoder(3) for _ in range(2))

            self.motion_hyperprior = nn.ModuleList(HyperpriorMasked(factor = 1, 
                                               mask_policy=mask_policy,
                                               scalable_levels=scalable_levels) for _ in range(2))
                
        self.res_hyperprior = nn.ModuleList(HyperpriorMasked(factor = 1, 
                                               mask_policy=mask_policy,
                                               scalable_levels=scalable_levels) for _ in range(2))
    ...
    def encode_inter(self, x_cur, x_ref, quality, mask_pol):
        ...
        if quality == 0:
            ...
            y_res_hat, out_res = self.res_hyperprior[1].compress(y_res,quality,mask_pol)
