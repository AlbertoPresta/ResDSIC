...
class ScalableDistilledDistortionLoss(nn.Module):
    ...
    def forward(self,output,target,lmbda = None): 
        ...
        if "y_prog" in list(likelihoods.keys()):
            ...
            out["bpp_base"] = (1 - self.gamma)*out["kd_base"] + self.gamma*((torch.log(likelihoods["y"]).sum())/denominator)
            out["bpp_scalable"] = (1 - self.gamma)*out["kd_enh"] + self.gamma*((torch.log(likelihoods["y_prog"]).sum()).sum()/denominator) 
            out["bpp_loss"] = out["bpp_scalable"] +  out["bpp_base"] + batch_size_recon*(out["bpp_hype"])
        else: 
            out["kd_base"] = kd_base
            out["kd_enh"] = kd_enh
